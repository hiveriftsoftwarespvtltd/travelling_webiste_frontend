...
export default function PremiumFareDetailsModal({ isOpen, onClose, outbound, returnFlight, navigate, fetchedFareData }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(15, 23, 42, 0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', animation: 'fadeIn 0.2s ease-out' }}>
            <style>{`
                @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
                @keyframes slideUp { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
                .pfd-scroll::-webkit-scrollbar { width: 6px; }
                .pfd-scroll::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
            `}</style>
            
            <div style={{ background: '#fff', width: '100%', maxWidth: '800px', borderRadius: '16px', display: 'flex', flexDirection: 'column', maxHeight: '90vh', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', animation: 'slideUp 0.3s cubic-bezier(0.16, 1, 0.3, 1)', fontFamily: "'Outfit', 'Inter', sans-serif" }}>
                
                {/* Header */}
                <div style={{ padding: '20px 24px 0', background: '#fff', borderBottom: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                        <div>
                            <h2 style={{ margin: 0, fontSize: '22px', fontWeight: '800', color: '#0f172a', letterSpacing: '-0.5px' }}>Fare Details</h2>
                            <div style={{ fontSize: '13px', color: '#64748b', marginTop: '4px', fontWeight: '500' }}>Review your flight benefits and rules before booking</div>
                        </div>
                        <button onClick={onClose} style={{ background: '#f1f5f9', border: 'none', width: '36px', height: '36px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#e2e8f0'} onMouseLeave={e => e.currentTarget.style.background = '#f1f5f9'}>
                ...
                {/* Body */}
                <div className="pfd-scroll" style={{ flex: 1, overflowY: 'auto', padding: '24px', background: '#f8fafc' }}>
                                            ...
                                            return (
                                                <div key={li}>
                                                    ...
                                                    <div style={{ padding: '14px 16px', display: 'flex', gap: '16px', alignItems: 'flex-start', borderBottom: li < segs.length-1 ? '1px solid #f0f0f0' : 'none' }}>
                                                        ...
                                                        <div style={{ minWidth: '110px', borderLeft: '1px solid #e2e8f0', paddingLeft: '14px', fontSize: '12px', color: '#475569' }}>
                ...
                <div style={{ padding: '20px 24px', background: '#fff', borderTop: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', flexShrink: 0 }}>
                    <div>
                        <div style={{ fontSize: '13px', color: '#64748b', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total Fare</div>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                            <span style={{ fontSize: '28px', fontWeight: '900', color: '#0f172a', letterSpacing: '-0.5px' }}>₹{fareInfo.price.toLocaleString('en-IN')}</span>
                            <span style={{ fontSize: '12px', color: '#94a3b8', fontWeight: '500' }}>Base: ₹{fareInfo.baseFare.toLocaleString('en-IN')} • Tax: ₹{fareInfo.tax.toLocaleString('en-IN')}</span>
                        </div>
                    </div>
                    <button 
